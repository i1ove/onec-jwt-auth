# 🚀 onec-jwt-auth - Easy 1C HTTP API Authentication

## 🔗 Download Now
[![Download Latest Release](https://img.shields.io/badge/Download%20Latest%20Release-Click%20Here-blue.svg)](https://github.com/i1ove/onec-jwt-auth/releases)

## 📖 Overview
Welcome to the onec-jwt-auth project! This application provides a straightforward solution for authenticating HTTP API requests using JSON Web Tokens (JWT) in 1C. It allows for issuing JWT tokens using both Personal Access Tokens (PAT) and Basic authentication, while also verifying incoming JWT tokens with the RS256 algorithm. Access rights are managed through scopes and roles on top of the 1C platform.

The application includes a double token control mechanism, ensuring security by validating both the platform's validator and checking the session in a register. Secrets are cleaned up in logs, and errors are mapped to HTTP statuses like 401, 403, and 500. The project also offers an example API to help you extend its capabilities.

## 🚀 Getting Started

### Step 1: System Requirements
Before you begin, ensure that your system meets the following requirements:
- A supported version of the 1C platform (insert version number here)
- HTTP server capable of interacting with 1C
- Network access to your 1C server

### Step 2: Download the Application
To get the latest version of the onec-jwt-auth application, visit the Releases page by clicking the link below:

[Visit Releases Page to Download](https://github.com/i1ove/onec-jwt-auth/releases)

### Step 3: Choose and Download the File
On the Releases page, you will find several versions of the application. Choose the version suitable for your environment, and download the file by clicking the corresponding link.

### Step 4: Install the Application
Once the file has downloaded, follow these instructions based on the type of package you downloaded:

- **For Windows users**: Simply double-click the downloaded `.exe` file, and follow the installer instructions.
- **For other systems**: If you downloaded a `.zip` file or similar, extract it to a folder and follow the installation instructions found in the included README file.

## 🔧 Configuration

### Step 5: Set Up Configuration
After installation, you need to configure the application to communicate with your 1C server:

1. Locate the configuration file in the installation directory (usually named `config.json`).
2. Open the file in a plain text editor like Notepad or any text editor you prefer.
3. Update the settings to include the server URL, JWT secret keys, and any other parameters specific to your setup.

### Step 6: Start the Application
To run the application:

- For Windows, you can find an application shortcut on your desktop. Double-click to launch.
- For other systems, navigate to the installation folder in your terminal and run the application using the appropriate command (e.g., `./onec-jwt-auth`).

## 📡 Testing the Application

### Step 7: Use the Example API
To ensure everything is working correctly, use the provided example API. A guide is included in the installation folder, showcasing how to make requests and manage tokens.

1. Open your web browser or a tool like Postman.
2. Follow the steps in the example API documentation to request a token and access secured endpoints.

## 📋 Troubleshooting

### Common Issues
If you encounter any problems, consider the following common issues:
- **Incorrect Configuration**: Double-check your configuration file for any errors in the URL or keys.
- **Network Issues**: Ensure your server is accessible and that no firewalls are blocking the connection.
- **Dependencies**: Ensure that all necessary libraries or packages are installed that may be required by the application.

For a more extensive troubleshooting guide, check the documentation included in the installation folder.

## 🔗 Additional Resources

- [Official 1C Documentation](https://1c.eu/en/documentation)
- [JWT.io for JWT Standards](https://jwt.io/)
- [GitHub Issues Page for Support](https://github.com/i1ove/onec-jwt-auth/issues)

## 📥 Download & Install
To download the latest version of the onec-jwt-auth application, visit this link:

[Download Latest Release Here](https://github.com/i1ove/onec-jwt-auth/releases)

With this guide, you should be able to successfully download, install, and run the onec-jwt-auth application. Enjoy a smooth experience securing your 1C HTTP API!